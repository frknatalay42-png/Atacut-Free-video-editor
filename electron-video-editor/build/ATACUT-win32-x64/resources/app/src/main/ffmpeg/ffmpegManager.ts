...
// Set FFmpeg paths - use the actual .exe files
function getFFmpegPath(): string {
  ...
  const distPath = path.join(app.getAppPath(), '../ffmpeg.exe');
  if (existsSync(distPath)) {
    return distPath;
  }

  // Try resources/app folder (for packaged app from ASAR)
  const resourcesPath = path.join(app.getAppPath(), 'resources/app/ffmpeg.exe');
  if (existsSync(resourcesPath)) {
    return resourcesPath;
  }

  // Fallback to node_modules
  return path.join(app.getAppPath(), 'node_modules/ffmpeg-static/ffmpeg.exe');
}
...
function getFFprobePath(): string {
  ...
  // Try dist/bin folder
  const distPath = path.join(app.getAppPath(), '../bin/win32/x64/ffprobe.exe');
  if (existsSync(distPath)) {
    return distPath;
  }

  // Try resources/app folder
  const resourcesPath = path.join(app.getAppPath(), 'resources/app/bin/win32/x64/ffprobe.exe');
  if (existsSync(resourcesPath)) {
    return resourcesPath;
  }

  // Fallback to node_modules
  return path.join(app.getAppPath(), 'node_modules/ffprobe-static/bin/win32/x64/ffprobe.exe');
}
