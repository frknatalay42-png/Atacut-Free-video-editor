...
import { motionTrackingManager } from '../tracking/motionTrackingManager';
...
export function setupIpcHandlers(mainWindow: BrowserWindow) {
  ...
  ipcMain.handle('EXPORT_VIDEO', async (event, exportData: any) => {
    try {
      ...
      // Determine target export settings (resolution, fps, bitrate)
      const defaultWidth = 1920;
      const defaultHeight = 1080;
      const settingsObj = exportData?.settings || {};
      const resolutionStr = settingsObj.resolution || `${defaultWidth}x${defaultHeight}`;
      const matchRes = /^(\d+)x(\d+)$/i.exec(resolutionStr);
      const targetWidth = matchRes ? parseInt(matchRes[1], 10) : defaultWidth;
      const targetHeight = matchRes ? parseInt(matchRes[2], 10) : defaultHeight;
      const targetFps = typeof settingsObj.fps === 'number' ? settingsObj.fps : 30;
      const targetBitrate = settingsObj.bitrate || (targetWidth >= 3840 ? '40000k' : '8000k');
      ...
      return new Promise((resolve, reject) => {
        ...
        const buildTextOverlay = (textClips: any[]): string => {
          ...
          for (const textClip of textClips) {
            ...
            // Convert position to pixels using target resolution
            ...
            const x = Math.round(targetWidth * posX);
            const y = Math.round(targetHeight * posY);
        ...
        if (n === 1) {
          ...
          if (colorFilter === '[0:v]' && !textOverlay) {
            ...
            const videoFilter = colorFilter === '[0:v]' 
              ? `[0:v]scale=${targetWidth}:${targetHeight}[vout]`  // No effects - just scale and label
              : `${colorFilter},scale=${targetWidth}:${targetHeight}[vout]`;        // Has effects - add output label
          ...
          // After concat, scale the resulting video to target resolution
          const filterComplexBase = `${videoFilter};${audioFilter}`;
          const filterComplex = `${filterComplexBase};[vout]scale=${targetWidth}:${targetHeight}[vout_scaled]`;
          const textOverlay = buildTextOverlay(textClips);
          ...
          if (textOverlay) {
            finalFilter = `${filterComplex};[vout_scaled]${textOverlay}[vout_final]`;
          }
          ...
          command
            ...
            .format('mp4')
            .outputOptions([
              '-map', textOverlay ? '[vout_final]' : '[vout_scaled]',
              '-map', '[aout]',
              '-c:v', 'libx264',
              '-preset', 'medium',
              '-crf', '23',
  ...
  ipcMain.handle(IPC_CHANNELS.ANALYZE_MOTION, async (event, payload) => {
    try {
      const { videoPath, trackingType, startFrame, endFrame, options } = payload;
      logger.info('Starting motion tracking analysis', { trackingType });

      const trackingData = await motionTrackingManager.analyzeMotion(
        videoPath,
        trackingType,
        startFrame,
        endFrame,
      ...
      motionTrackingManager.on('tracking-progress', (progress) => {
        mainWindow.webContents.send('tracking-progress', progress);
      });

      motionTrackingManager.on('tracking-complete', (data) => {
        mainWindow.webContents.send('tracking-complete', data);
      });
  ...
  ipcMain.handle(IPC_CHANNELS.CANCEL_MOTION_TRACKING, async (event, trackId: string) => {
    try {
      motionTrackingManager.cancelTracking(trackId);
      return { success: true };
    } catch (error) {
      logger.error('Failed to cancel tracking', error);
      return { success: false, error: (error as Error).message };
    }
  });

  ipcMain.handle(IPC_CHANNELS.GET_TRACKING_DATA, async (event, trackId: string) => {
    try {
      const data = motionTrackingManager.getTrackingData(trackId);
  ...
  ipcMain.handle(IPC_CHANNELS.APPLY_TRACKING_KEYFRAMES, async (event, payload) => {
    try {
      const { clipId, trackingData, targetElement } = payload;
      logger.info('Applying tracking keyframes', { clipId, targetElement });

      // Convert tracking bounds to keyframes and apply to clip
      const keyframes = trackingData.bounds.map((bound, index) => ({
        id: `kf-${index}`,
        ...
        scaleX: 1,
        scaleY: 1,
        rotation: bound.rotation || 0,
        easing: 'linear',
      }));

      return { success: true, keyframeCount: keyframes.length, keyframes };
    } catch (error) {
      logger.error('Failed to apply tracking keyframes', error);
      return { success: false, error: (error as Error).message };
    }
  });
