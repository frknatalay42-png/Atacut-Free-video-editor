...
const App: React.FC = () => {
  ...
  // Export settings (simple selector for resolution)
  const [exportResolution, setExportResolution] = useState<'1080p' | '4k'>('1080p');

  // Project Preset Modal (platform sizes)
  type ProjectPreset = { name: string; resolution: string; width: number; height: number; fps: number; bitrate: string };
  const PLATFORM_PRESETS: Record<string, ProjectPreset[]> = {
    youtube: [
      { name: 'YouTube 4K (2160p)', resolution: '3840x2160', width: 3840, height: 2160, fps: 30, bitrate: '40000k' },
      { name: 'YouTube 1080p', resolution: '1920x1080', width: 1920, height: 1080, fps: 30, bitrate: '8000k' },
      { name: 'YouTube 720p', resolution: '1280x720', width: 1280, height: 720, fps: 30, bitrate: '5000k' },
    ],
    instagram: [
      { name: 'Instagram Story/Reel (Vertical)', resolution: '1080x1920', width: 1080, height: 1920, fps: 30, bitrate: '12000k' },
      { name: 'Instagram Feed (Square)', resolution: '1080x1080', width: 1080, height: 1080, fps: 30, bitrate: '10000k' },
    ],
    facebook: [
      { name: 'Facebook 1080p (Landscape)', resolution: '1920x1080', width: 1920, height: 1080, fps: 30, bitrate: '8000k' },
      { name: 'Facebook Square', resolution: '1080x1080', width: 1080, height: 1080, fps: 30, bitrate: '8000k' },
      { name: 'Facebook Vertical', resolution: '1080x1920', width: 1080, height: 1920, fps: 30, bitrate: '12000k' },
    ],
    tiktok: [
      { name: 'TikTok Vertical', resolution: '1080x1920', width: 1080, height: 1920, fps: 30, bitrate: '12000k' },
    ],
  };
  const [showProjectPresetModal, setShowProjectPresetModal] = useState(false);
  const [projectPlatform, setProjectPlatform] = useState<'youtube' | 'instagram' | 'facebook' | 'tiktok'>('youtube');
  const [projectPreset, setProjectPreset] = useState<ProjectPreset | null>(null);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // Choose export settings based on selected project preset or manual dropdown
      const chosenSettings = projectPreset
        ? {
            codec: 'h264',
            resolution: projectPreset.resolution,
            fps: projectPreset.fps,
            bitrate: projectPreset.bitrate,
          }
        : exportResolution === '4k'
          ? {
              codec: 'h264',
  ...
  return (
    ...
    >
      ...
      <div style={{ 
        ...
        <button
          onClick={() => setShowProjectPresetModal(true)}
          style={{
            ...smallButtonStyle,
            background: '#3a3a3a',
            marginLeft: '8px',
          }}
          title="Create New Project (choose platform sizes)"
        >
          🆕 New Project
        </button>
        {/* Resolution selector */}
        <select
          ...
          onChange={(e) => setExportResolution((e.target.value === '4k' ? '4k' : '1080p'))}
          style={{
            marginLeft: '12px',
            marginRight: '8px',
            padding: '6px 10px',
        ...
        {showProjectPresetModal && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
            <div style={{ width: '800px', maxWidth: '95vw', background: '#1f1f1f', border: '1px solid #444', borderRadius: 12, boxShadow: '0 12px 24px rgba(0,0,0,0.4)' }}>
              <div style={{ padding: '16px 20px', borderBottom: '1px solid #333', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', gap: 8 }}>
                  {(['youtube','instagram','facebook','tiktok'] as const).map(p => (
                    <button key={p} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #444', background: projectPlatform === p ? '#9B59B6' : '#2b2b2b', color: '#fff', cursor: 'pointer' }} onClick={() => setProjectPlatform(p)}>{p.charAt(0).toUpperCase() + p.slice(1)}</button>
                  ))}
                </div>
                <button style={{ ...smallButtonStyle, background: '#3a3a3a' }} onClick={() => setShowProjectPresetModal(false)}>✖</button>
              </div>
              <div style={{ padding: 20 }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}>
                  {PLATFORM_PRESETS[projectPlatform].map((preset) => {
                    const selected = projectPreset?.resolution === preset.resolution && projectPreset?.name === preset.name;
                    return (
                      <div key={preset.name} style={{ padding: 12, border: '1px solid ' + (selected ? '#9B59B6' : '#444'), borderRadius: 10, background: selected ? 'rgba(155,89,182,0.15)' : '#262626', cursor: 'pointer' }} onClick={() => setProjectPreset(preset)}>
                        <div style={{ fontWeight: 600, marginBottom: 6 }}>{preset.name}</div>
                        <div style={{ color: '#bbb' }}>{preset.resolution} • {preset.fps} fps</div>
                        <div style={{ color: '#aaa', marginTop: 4 }}>Bitrate: {preset.bitrate}</div>
                      </div>
                    );
                  })}
                </div>
              </div>
              <div style={{ padding: 16, borderTop: '1px solid #333', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                <button
                  style={{ ...buttonStyle, background: projectPreset ? 'linear-gradient(135deg, #9B59B6, #7D3C98)' : '#3a3a3a', cursor: projectPreset ? 'pointer' : 'not-allowed' }}
                  disabled={!projectPreset}
                  onClick={() => {
                    if (!projectPreset) return;
                    if (projectPreset.width >= 3840) setExportResolution('4k'); else setExportResolution('1080p');
                    setShowProjectPresetModal(false);
                  }}
                >
                  Create Project
                </button>
